...
namespace EdFi.Tools.ApiPublisher.Core.Configuration.Enhancers
{
    public class NamedConnectionsConfigurationBuilderEnhancer : IConfigurationBuilderEnhancer
    {
        ...
        public void Enhance(IConfigurationRoot initialConfiguration, IConfigurationBuilder configurationBuilder)
        {
            ...
            {
                _logger.Debug(
                    $"Obtaining {connectionType.ToString().ToLower()} API connection details for connection '{apiConnectionName}' using '{_namedApiConnectionDetailsReader.GetType().Name}'.");
                ...
                {
                    throw new ArgumentException(
                        $"Named {connectionType.ToString().ToLower()} connection '{namedApiConnectionDetails.Name}' was not fully configured. The following values are missing: [{string.Join(", ", namedApiConnectionDetails.MissingConfigurationValues())}]");
                }
                ...
                if (namedApiConnectionDetails.TreatForbiddenPostAsWarning.HasValue && connectionType == ConnectionType.Target)
                {
                    yield return new KeyValuePair<string, string>(
