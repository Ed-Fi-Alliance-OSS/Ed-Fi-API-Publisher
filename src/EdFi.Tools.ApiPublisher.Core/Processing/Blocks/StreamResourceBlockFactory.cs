...
namespace EdFi.Tools.ApiPublisher.Core.Processing.Blocks
{
    public class StreamResourceBlockFactory
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            // StreamResourceMessage per resource and its output holds only lightweight page messages
            // (offset/limit, or one partition token each under cursor paging), while its delegate performs
            // the dependency waits and processing-semaphore acquisition below -- bounding it would risk
            // interfering with that coordination for no memory benefit.
