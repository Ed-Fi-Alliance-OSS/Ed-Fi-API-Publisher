...
namespace EdFi.Tools.ApiPublisher.Connections.Api.ApiClientManagement
{
    ...
    public class EdFiApiUrlSegmentResolver
    {
        ...
        {
            // Both forms are checked because a placeholder survives a round trip through Uri as its escaped
            // spelling, so looking for the braces alone would let the escaped one through.
            string[] placeholderMarkers = ["{", "}", "%7B", "%7D"];

            if (!placeholderMarkers.Any(marker => segment.Contains(marker, StringComparison.OrdinalIgnoreCase)))
            {
                return segment;
            }

            throw new InvalidOperationException(
                $"The {definition.DiscoveryUrlName} path for the {_connectionName} connection resolved to '{segment}', which still carries a route placeholder. An API that qualifies its routes by tenant or school year resolves them only for an address that names one, so the URL for this connection has to include that prefix (for example 'https://server/tenant/') rather than the server root."
            );
        }
