...
{
    public class PostResourceBlocksFactory : IProcessDataPipelineFactory<PostItemMessage>
    {
            ...
            CreateBlocksRequest createBlocksRequest)
        {
            ...
            var postResourceBlock = new TransformManyBlock<PostItemMessage, ErrorItemMessage>(
                async msg =>
                {
                    ...
                    try
                    {
                        ...
                        {
                            _logger.Debug(
                                $"{msg.ResourceUrl} (source id: {id}): Processing PostItemMessage (with up to {options.MaxRetryAttempts} retries).");
                        }
                            ...
                            .OrResult<HttpResponseMessage>(
                                r =>
                                    ...
                                    (r.StatusCode == HttpStatusCode.Conflict
                                        && !msg.ResourceUrl.EndsWith("Descriptors", StringComparison.OrdinalIgnoreCase))
                                    ...
                                    || (!r.IsSuccessStatusCode
                                        ...
                                        && MayHaveRemediation(msg.ResourceUrl, r.StatusCode)))
                            .WaitAndRetryAsync(
                                delay,
                                async (result, ts, retryAttempt, ctx) =>
                                {
                                    if (javaScriptModuleFactory != null)
                                    {
                                            ...
                                            && modifiedRequestBody.ValueKind != JsonValueKind.Null)
                                        {
                                            if (_logger.IsDebugEnabled)
                                            {
                                                ...
                                                _logger.Debug(
                                                    $"{msg.ResourceUrl} (source id: {id}): Remediation plan provided a modified request body: {modifiedRequestBodyJson} ");
                                            }
                                    ...
                                    {
                                        _logger.Error(
                                            $"{msg.ResourceUrl} (source id: {id}): POST attempt #{attempts} failed with an exception. Retrying... (retry #{retryAttempt} of {options.MaxRetryAttempts} with {ts.TotalSeconds:N1}s delay):{Environment.NewLine}{result.Exception}");
                                    }
                                    else
                                    {
                                        ...
                                        _logger.Warn(
                                            $"{msg.ResourceUrl} (source id: {id}): POST attempt #{attempts} failed with status '{result.Result.StatusCode}'. Retrying... (retry #{retryAttempt} of {options.MaxRetryAttempts} with {ts.TotalSeconds:N1}s delay):{Environment.NewLine}{responseContent}");
                                    }
                                })
                            .ExecuteAsync(
                                async (ctx, ct) =>
                                {
                                    ...
                                    {
                                        _logger.Info(
                                            $"{msg.ResourceUrl} (source id: {id}): Applying modified request body from remediation plan...");
                        ...
                        if (!apiResponse.IsSuccessStatusCode)
                        {
                            ...
                            if (msg.ResourceUrl.EndsWith("Descriptors") && apiResponse.StatusCode == HttpStatusCode.Conflict)
                            {
                                if (_logger.IsDebugEnabled)
                                {
                                    _logger.Debug(
                            ...
                            if (apiResponse.StatusCode == HttpStatusCode.Forbidden)
                            {
                                ...
                                if (msg.PostAuthorizationFailureRetry != null)
                                {
                                    ...
                                    {
                                        _logger.Debug(
                                            $"{msg.ResourceUrl} (source id: {id}): Authorization failed -- deferring for retry after pertinent associations are processed.");
                                    }
                                ...
                                && targetEdFiApiClient.ConnectionDetails?.TreatForbiddenPostAsWarning == true)
                            {
                                ...
                                _logger.Warn(
                                    $"{msg.ResourceUrl} (source id: {id}): Authorization failed on POST of resource with no authorization failure handling defined. Remaining resource items will be ignored. Response status: {apiResponse.StatusCode}{Environment.NewLine}{responseContent}");
                            ...
                            _logger.Error(
                                $"{msg.ResourceUrl} (source id: {id}): POST attempt #{attempts} failed with status '{apiResponse.StatusCode}':{Environment.NewLine}{responseContent}");
                    ...
                    {
                        _logger.Error(
                            $"{msg.ResourceUrl} (source id: {id}): An unhandled exception occurred in the PostResource block: {ex}");
            ...
            bool IsBadRequestForUnresolvedReferenceOfPrimaryRelationship(
                HttpResponseMessage postItemResponse,
                PostItemMessage msg)
            {
                        ...
                        if (responseMessageText?.Contains("reference could not be resolved.") == true)
                        {
                            ...
                            if (referenceNameMatch.Success)
                            {
                                ...
                                else
                                {
                                    ...
                                    _logger.Error(
                                        $"{msg.ResourceUrl}: GET by Id request from source API for missing '{referencedResourceName}' reference failed with status '{getByIdResponse.StatusCode}': {responseContent}");
                                }
